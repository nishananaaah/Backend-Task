...
router.get("/search", async (req, res) => {
  try {
    ...
    await Contact.find({
      //match these any of the fields return the data
      $or: [
      ...
      .then((contacts) => {
        if (contacts.length) {
          ...
          res
            .status(500)
            .json({ contact: [], message: "No matching record found" });
        }
        res.status(200).json({ contacts: contacts });
      })
      .catch((error) => {
...
//2-Types
//1-Soft Delete => update the field is "active" => Y/N
...
router.delete("/contact/:id", async (req, res) => {
  try {
    ...
    .catch((error)=>{
        console.log(error)
        res.status(500).json({message:"Unable to delete the contact"})
    })
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Unable to delete the contact" });
